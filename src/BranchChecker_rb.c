...
void inorderTraversal(struct rbNode *node, int* result) {
    if (node == NULL) return;
    if (curIndexRB >= 11) return;

    inorderTraversal(node->link[0], result);
    ...
    if(node->data != 0) {
        result[curIndexRB] = node->data;
        curIndexRB++;
    }

    inorderTraversal(node->link[1], result);
  return;
}
