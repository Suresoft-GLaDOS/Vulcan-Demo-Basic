...
#include "ShortestSuper.h"

int tailLength(const char *a, int la, const char *b, int lb) {
    int i = 0;
...
#define IDX(I, J) ((I) * ASize + (J))
    ...
    for (i = 0; i < ASize; i ++) {
        for (j = i + 1; j < ASize; j ++) {
            ...
            addition[IDX(i, j)] = tailLength(A[i], len[i], A[j], len[j]);
            addition[IDX(j, i)] = tailLength(A[j], len[j], A[i], len[i]);
        }
    }
